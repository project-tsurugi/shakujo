...
bool is_boolean(Type const* type) {
    return type->kind() == Type::Kind::BOOL;
}

bool is_numeric(Type const* type) {
    return is_valid(type) && (is_integral(type) || type->kind() == Type::Kind::FLOAT);
}

bool is_integral(Type const* type) {
    return is_valid(type) && type->kind() == Type::Kind::INT;
}

bool is_textual(Type const* type) {
    return is_valid(type) && (type->kind() == Type::Kind::STRING || type->kind() == Type::Kind::CHAR);
}

bool is_atom(Type const* type) {
    return is_valid(type) && (is_boolean(type) || is_numeric(type) || is_textual(type));
}

bool is_boolean_convertible(common::core::Type const* type) {
    // FIXME: boolean convertible
    return is_atom(type);
}

bool is_equality_comparable(common::core::Type const* a, common::core::Type const* b) {
...
bool is_order_comparable(common::core::Type const* a, common::core::Type const* b) {
...
std::unique_ptr<Type> binary_promotion(Type const* a, Type const* b) {
...
std::unique_ptr<Type> nullity(Type const* type, Type::Nullity nullity) {
    if (!is_valid(type)) {
        return std::make_unique<Error>();
    }
...
bool is_assignment_convertible(Type const* variable, binding::ExpressionBinding const& expression) {
...
std::unique_ptr<common::core::Type> convert(model::type::Type const* type) {
